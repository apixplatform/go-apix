		...
		if (!this.data) {
			this.data = [
				...
				// },
				{
					time: new Date().getDate() + '/' + (new Date().getMonth()+1),
					icon: 'fa fa-genderless kt-font-info',
					text: 'You are eligible for a loan up to <a href="$event.preventDefault();" class="kt-link kt-link--brand kt-font-bolder">USD 15000</a>. Check Loans section for more info.',
				},
				{
					time: new Date().getDate() + '/' + (new Date().getMonth()+1),
					icon: 'fa fa-genderless kt-font-success',
					text: 'Received <a href="$event.preventDefault();" class="kt-link kt-link--brand kt-font-bolder">USD 10</a> as a gift from Go Bank',
				},
				// {
				// 	time: '16:00',
